    ...
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-4 px-4 py-6 sm:px-6">
      ...
      <ul className="flex flex-col gap-1.5">
        {(categorias ?? []).map((c) => (
          <li
            key={c.id}
            className="flex items-center justify-between gap-3 rounded-lg border border-borde bg-superficie px-4 py-2.5"
          >
            <span className={c.activo ? "text-tinta" : "text-tinta-etiqueta line-through"}>
              {c.nombre}
            </span>
