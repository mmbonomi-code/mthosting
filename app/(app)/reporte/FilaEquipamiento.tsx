...
export default function FilaEquipamiento({
  ...
  return (
    <li
      className={`flex flex-wrap items-start gap-3 rounded-xl border-y border-r border-y-borde border-r-borde border-l-4 bg-superficie px-4 py-3 ${borde} ${
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm text-tinta-tenue">
          ...
          {equipo.codigo_reserva && (
            <span className="font-mono text-tinta-etiqueta"> · {equipo.codigo_reserva}</span>
          )}
        </p>
        {equipo.notas && (
          <p className="whitespace-pre-wrap text-sm text-tinta-etiqueta">{equipo.notas}</p>
        )}
      ...
      {puedeEscribir && (
        <div className="flex shrink-0 flex-wrap gap-2">
          ...
          <button
            ...
            className="rounded-lg px-2 py-1.5 text-xs text-tinta-etiqueta hover:bg-elevada hover:text-tinta-suave disabled:opacity-60"
          >
            Archivar
          </button>
        </div>
