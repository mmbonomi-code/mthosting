    ...
    <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-4 px-4 py-6 sm:px-6">
      ...
      {porAnuncio.size === 0 ? (
        <div className="rounded-xl border border-borde bg-superficie px-6 py-12 text-center">
          ...
          <p className="mt-1 text-sm text-tinta-etiqueta">
        ...
        <ul className="flex flex-col gap-4">
          {[...porAnuncio.entries()].map(([anuncio, delAnuncio]) => (
            <li
              key={anuncio}
              className="flex flex-col gap-3 rounded-xl border border-aviso-borde/60 bg-aviso-soft/20 p-4"
            >
              <div>
              ...
              <ul className="flex flex-col gap-1 text-sm">
                {delAnuncio.slice(0, 5).map((reserva) => (
                  <li key={reserva.id} className="flex flex-wrap gap-x-3 text-tinta-tenue">
                    ...
                    {reserva.cancelada && (
                      <span className="text-tinta-apagada">cancelada</span>
                    )}
                  </li>
                ))}
                {delAnuncio.length > 5 && (
                  <li className="text-tinta-etiqueta">
