...
export default async function Tarifas() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-4 py-6 sm:px-6">
      ...
      <section className="rounded-xl border border-borde-control bg-superficie p-4">
        <h2 className="mb-1 font-medium text-tinta">Vigentes hoy</h2>
        {vigentes.length === 0 ? (
          <p className="py-4 text-sm text-tinta-etiqueta">
            ...
            {Object.entries(ETIQUETA_AMBIENTES).map(([valor, etiqueta]) => {
              ...
              return (
                <div key={valor} className="flex flex-col gap-0.5">
                  <dt className="text-xs uppercase tracking-wide text-tinta-etiqueta">
                    {etiqueta}
                  </dt>
                  ...
                  {t && (
                    <dd className="text-xs text-tinta-etiqueta">
        ...
        <p className="mt-4 border-t border-borde-control pt-3 text-xs text-tinta-etiqueta">
          <strong className="text-tinta-tenue">Reglas de pago:</strong> se paga
        ...
        {puntuales.length > 0 && (
          <ul className="flex flex-col gap-2">
            {puntuales.map((t) => (
              <li
                key={t.id}
                className="flex flex-wrap items-center gap-x-4 rounded-lg border border-borde px-3 py-2 text-sm"
              >
                <span className="font-mono text-tinta-media">{t.depto?.codigo}</span>
      ...
      {historial.length > 0 && (
        <section className="flex flex-col gap-2">
          ...
          <ul className="flex flex-col gap-1 text-sm">
            {historial.map((t) => (
              <li key={t.id} className="flex flex-wrap gap-x-3 text-tinta-etiqueta">
                <span>
                  {t.depto?.codigo ??
