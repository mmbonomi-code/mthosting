...
function Seccion({
  ...
  return (
    <fieldset className="rounded-xl border border-borde p-4">
      <legend className="px-2 text-sm font-semibold uppercase tracking-wide text-tinta-tenue">
        {titulo}
      </legend>
...
function Casilla({
  ...
  return (
    <label className="flex items-center gap-3 py-1">
      ...
      />
      <span className="text-base text-tinta-media">{etiqueta}</span>
    </label>
  );
}
  ...
  return (
    <form action={enviar} className="flex flex-col gap-6">
      ...
      <Seccion titulo="Camas">
        ...
        <p className="text-xs text-tinta-etiqueta sm:col-span-2">
      ...
      {estado?.error && (
        <p
          role="alert"
          className="rounded-lg bg-error-soft px-4 py-3 text-sm text-error-text"
        >
          {estado.error}
        </p>
      )}
