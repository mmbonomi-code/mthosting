...
const OPCIONES: { valor: Destino; texto: string; activo: string }[] = [
  { valor: "mthosting", texto: "De MTHosting", activo: "border-exito bg-exito-soft text-exito-text" },
  { valor: "propietario", texto: "Del propietario", activo: "border-dato-borde bg-dato-soft text-dato-text" },
  { valor: "sin_asignar", texto: "Sin decidir", activo: "border-aviso bg-aviso-soft/40 text-aviso-text" },
];
        ...
        {OPCIONES.map((o) => (
          <button
            ...
            className={`min-h-9 rounded-md border px-3 text-xs font-medium transition-colors disabled:opacity-50 ${
              valor === o.valor
                ? o.activo
                : "border-borde-control bg-superficie text-tinta-tenue hover:bg-elevada"
            }`}
          >
            {o.texto}
          </button>
