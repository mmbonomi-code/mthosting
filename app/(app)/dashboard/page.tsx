...
function Barra({ fila }: { fila: OcupacionDepto }) {
  return (
    <div className="flex h-2 overflow-hidden rounded-full bg-elevada-hover">
      <div
        className="bg-exito"
...
function Numero({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <span className="text-xs text-tinta-etiqueta">{detalle}</span>
    </div>
  );
}
...
export default async function Dashboard({
  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-5 px-4 py-6 sm:px-6">
      ...
      {periodoIncompleto && (
        <p className="rounded-lg bg-aviso-soft/40 px-4 py-3 text-sm text-aviso-text-fuerte">
      ...
      <section className="flex flex-col gap-4 rounded-xl border border-borde bg-superficie p-4">
        ...
        <Barra fila={total} />
        <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm text-tinta-tenue">
          <span>
          ...
          {total.estadia_promedio !== null && (
            <span>
              Estadía promedio{" "}
              <span className="text-tinta-media">{total.estadia_promedio} noches</span>
            </span>
          )}
          {total.pct_cancelacion !== null && (
            <span>
              Cancelación{" "}
              <span className="text-tinta-media">{total.pct_cancelacion}%</span> (
              {total.canceladas})
            </span>
          )}
      ...
      <section className="flex flex-col gap-2">
        ...
        {ordenadas.length === 0 ? (
          <p className="py-3 text-sm text-tinta-apagada">
          ...
          <ul className="flex flex-col gap-2">
            {ordenadas.map((fila) => {
              ...
              return (
                <li
                  key={fila.depto_id}
                  className="flex flex-col gap-2 rounded-xl border border-borde bg-superficie px-4 py-3"
                >
                  <div className="flex flex-wrap items-baseline justify-between gap-x-3">
                    <span className="min-w-0 truncate font-medium text-tinta">
                      {depto?.codigo}
                      {depto?.barrio && (
                        <span className="font-normal text-tinta-etiqueta">
                          {" "}
                          · {depto.barrio}
                        </span>
                      )}
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-tinta-etiqueta">
                    <span className="tabular-nums">
