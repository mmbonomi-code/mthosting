...
export default async function Feriados() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-4 px-4 py-6 sm:px-6">
      <Link href="/tarifas" className="text-sm text-tinta-tenue hover:text-tinta">
      ...
      {(feriados ?? []).length === 0 ? (
        <p className="py-8 text-center text-tinta-etiqueta">
        ...
        <ul className="flex flex-col gap-2">
          {(feriados ?? []).map((f) => (
            <li
              key={f.id}
              className="flex flex-wrap items-center gap-x-4 rounded-lg border border-borde px-4 py-2.5"
            >
              <span className="w-28 shrink-0 text-tinta-media">
                {formatearFechaAR(f.fecha)}
              </span>
              <span className="min-w-0 flex-1 truncate text-sm text-tinta-tenue">
                {f.descripcion ?? ""}
              </span>
