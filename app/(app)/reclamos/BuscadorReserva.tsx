...
export default function BuscadorReserva() {
  ...
  return (
    <>
      ...
      {abierto && (
        <div
          className="fixed inset-0 z-20 flex items-start justify-center bg-fondo-hundido/70 p-4 pt-20"
          onClick={cerrar}
        >
          <div
            className="flex w-full max-w-lg flex-col overflow-hidden rounded-xl border border-borde-control bg-fondo"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="border-b border-borde px-4 py-3">
              <h2 className="text-xs font-semibold uppercase tracking-wider text-tinta-tenue">
                Buscar reserva
              </h2>
            </div>
            ...
            <div className="max-h-80 overflow-y-auto">
              {q.trim().length < 2 ? (
                <p className="px-4 pb-4 text-sm text-tinta-etiqueta">
                  Escribí al menos dos letras.
                </p>
              ) : buscando ? (
                <p className="px-4 pb-4 text-sm text-tinta-etiqueta">Buscando…</p>
              ) : resultados.length === 0 ? (
                <p className="px-4 pb-4 text-sm text-tinta-etiqueta">
                  ...
                  {resultados.map((r) => (
                    <li key={r.id} className="border-t border-borde">
                      <button
                        ...
                        className="w-full px-4 py-3 text-left transition-colors hover:bg-elevada"
                      >
                        <span className="flex flex-wrap items-center gap-2">
                          <span className="font-medium text-tinta">
                            {r.huesped_nombre ?? "Sin nombre"}
                          </span>
                          <span className="font-mono text-sm text-tinta-tenue">
                            {r.codigo_reserva}
                          </span>
                          {r.reclamo_id && (
                            <span className="rounded-full bg-elevada px-2 py-0.5 text-xs text-tinta-suave">
                              ya tiene reclamo
                            </span>
                          )}
                        </span>
                        <span className="mt-0.5 block text-sm text-tinta-etiqueta">
