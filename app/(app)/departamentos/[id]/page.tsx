...
function Acordeon({
  ...
  return (
    <details className="group rounded-xl border border-borde">
      <summary className="flex cursor-pointer items-center justify-between gap-3 px-4 py-3 text-tinta [&::-webkit-details-marker]:hidden">
        ...
        <span className="flex items-center gap-3">
          {resumen && (
            <span className="hidden max-w-64 truncate text-sm text-tinta-etiqueta sm:block">
              {resumen}
            </span>
          )}
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-4 px-4 py-6 sm:px-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-tinta-tenue">{depto.nombre_interno}</p>
        </div>
            ...
            <ul className="flex flex-col gap-1.5">
              {items.map((item) => (
                ...
                >
                  <span
                    className={
                      item.tiene ? "text-tinta-media" : "text-tinta-apagada"
                    }
                  >
